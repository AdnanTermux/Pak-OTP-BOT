...
# ── OTP GUI Theme ─────────────────────────────────────────────────
# 5 selectable themes. Stored in config.json as OTP_GUI_THEME (0-4).
...
def build_otp_msg(header: str, count_badge: str, clean: str,
    ...
    t = OTP_GUI_THEME % 5   # clamp to 0-4
    ...
    if t == 0:
        ...
        # ── Theme 4: ROYAL GOLD (crown + gold style) ─────────────────
        if for_group:
            if clean:
                return (f"👑 {flag} <b>#{svc}</b>  <code>{nd}</code> 🌟\n"
                        f"🔑 <code>{clean}</code>\n"
...
def admin_settings_kb() -> InlineKeyboardMarkup:
    theme_name = _THEME_NAMES.get(OTP_GUI_THEME % 5, "Unknown")
    return InlineKeyboardMarkup([
...
def gui_theme_kb() -> InlineKeyboardMarkup:
    ...
    for tid, name in _THEME_NAMES.items():
        mark = "✅ " if tid == OTP_GUI_THEME % 5 else ""
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="pick_gui":
        ...
        await query.edit_message_text(
            ...
            f"<b>1</b> — Screenshot Style  (flag + number + 🔥)\n"
            f"<b>2</b> — Neon / Electric  (⚡ bold + dashes)\n"
            f"<b>3</b> — Sigma Classic  (━ lines + OTP box)\n"
            f"<b>4</b> — Minimal Clean  (no decoration)\n"
            f"<b>5</b> — Royal Gold  (👑 crown + stars)",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"{'✅' if cur==1 else '1️⃣'}  Screenshot", callback_data="gui_set_1"),
                 InlineKeyboardButton(f"{'✅' if cur==2 else '2️⃣'}  Neon",       callback_data="gui_set_2")],
                [InlineKeyboardButton(f"{'✅' if cur==3 else '3️⃣'}  Sigma",      callback_data="gui_set_3"),
                 InlineKeyboardButton(f"{'✅' if cur==4 else '4️⃣'}  Minimal",    callback_data="gui_set_4")],
                [InlineKeyboardButton(f"{'✅' if cur==5 else '5️⃣'}  Royal Gold", callback_data="gui_set_5")],
    ...
    if data.startswith("gui_set_"):
        ...
        GUI_STYLE = style
        save_config_key("OTP_GUI_THEME", style - 1)  # theme index is 0-based
