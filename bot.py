...
async def do_sms_hit(bot_app,db_obj,otp_code,msg_body,service_name,panel_name,num_raw,session):
    ...
    # Number display: "2250•BOT•7623" style matching screenshot
    # Use first digits of dial code + bullet + bot name short + bullet + last4
    bot_short = (BOT_USERNAME or "SIGMA").lstrip("@").upper()[:6]
    num_display = f"{dial}•{bot_short}•{last5}"

    # ══ DM MESSAGE — matches screenshot style ═══════════════════════
    if clean:
        dm_txt = (
            f"{flag} #{region}  📱 <code>{num_display}</code>  🔥\n"
            f"<b>— {html.escape(bot_tag)}</b>\n\n"
            f"<b>OTP Received</b>  ·  {count_badge}\n"
            f"🔑 <code>{clean}</code>\n\n"
            f"📡 #{svc}   🔌 <code>{html.escape(panel_name)}</code>\n"
            f"💬 <i>{html.escape(msg_body[:260])}</i>\n\n"
        ...
        dm_txt = (
            ...
            f"<b>— {html.escape(bot_tag)}</b>\n\n"
            f"📩 New SMS  ·  📡 #{svc}\n"
            f"🔌 <code>{html.escape(panel_name)}</code>\n\n"
    ...
    if clean:
        ...
        grp_txt = (
            f"{flag} #{region}  📱  <code>{num_display}</code>\n"
            f"📡 #{svc}   🔌 <code>{html.escape(panel_name)}</code>\n"
            f"💬 <i>{html.escape(msg_body[:160])}</i>\n\n"
            f"<i>©By {html.escape(bot_tag)}</i>"
        )
...
async def log_unassigned(bot_app,num_raw,msg_body,otp_code,service_name,panel_name):
    ...
    if clean:
        txt = (
            f"⚪ <b>━━ UNASSIGNED OTP ━━</b>  {flag}\n"
            f"📱 <code>{dial}-SIGMA-{last5}</code>  📡 #{svc}\n"
            f"🔑 <code>{clean}</code>\n"
            f"🔌 {html.escape(panel_name)}  ⏰ <code>{now_ts}</code>\n"
            f"🤖 <b>{html.escape(bot_tag)}</b>"
        )
    else:
        txt = (
            f"⚪ <b>━━ UNASSIGNED SMS ━━</b>  {flag}\n"
            f"📱 <code>{dial}-SIGMA-{last5}</code>  📡 #{svc}\n"
            f"🔌 {html.escape(panel_name)}  ⏰ <code>{now_ts}</code>\n"
            f"💬 <i>{html.escape(msg_body[:160])}</i>\n"
            f"🤖 <b>{html.escape(bot_tag)}</b>"
