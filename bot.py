...
# ═══════════════════════════════════════════════════════════
_OTP_RE = [
    r"(?:OTP|code|verification\s*code|passcode|pin)[\s:]+([0-9]{4,8})",
    r"(?:FB|IG|WA|TG|SC|TT|G)[\s\-]([0-9]{4,8})",
    r"msverify[\s:]*([0-9]{4,8})",
    r"\b([0-9]{6})\b", r"\b([0-9]{5})\b", r"\b([0-9]{4})\b",
    r"\b([0-9]{8})\b", r"\b([0-9]{4}[-\s][0-9]{4})\b",
    r"\b([0-9]{3}[-\s][0-9]{3})\b",
]
...
class PanelSession:
    ...
    async def get_session(self) -> aiohttp.ClientSession:
        if self._session is None or self._session.closed:
            # Always recreate with THIS instance's cookie jar so cookies from
            # account A on panel X never bleed into account B on panel X.
...
def otp_keyboard(otp: Optional[str], full_msg: str = "",
    ...
    """
    OTP keyboard matching the DayzDigital-style GUI from the screenshot:
      • Green copy button:  >>  📋  OTP-DIGITS  (one-tap copy)
      • Blue PANEL button:  opens NUMBER_BOT_LINK / GET_NUMBER_URL
      • Blue INFO  button:  opens OTP_GROUP_LINK
    Both DM and group get the same button layout — only the copy button
    is omitted in groups (Telegram groups are read-only for bots).
    """
    clean = re.sub(r"[^0-9]", "", otp) if otp else ""
    kb = []

    # ── Row 1: OTP copy (green-style, both DM and group show this) ──────
    if clean:
        kb.append([
    ...
    # ── Row 2: SMS copy (DM only) ────────────────────────────────────────
    if not for_group and full_msg:
        kb.append([
            InlineKeyboardButton(
                "📩  Copy Full SMS",
                copy_text=CopyTextButton(text=full_msg[:256])),
        ])

    # ── Row 3: PANEL + INFO  (blue row — matches screenshot exactly) ─────
    # PANEL = opens the number bot (where users get numbers)
    # INFO  = opens OTP group / channel
    ...
    action_row = []
    if panel_url:
        action_row.append(InlineKeyboardButton("📱  PANEL  ↗", url=panel_url))
    if info_url:
        action_row.append(InlineKeyboardButton("💬  INFO  ↗",  url=info_url))
    if action_row:
        ...
        try:
            ...
            try:
                async with websockets.connect(panel.uri,ssl=ctx) as ws:
                    ...
                    try:
                        while True:
                            if panel.id not in [p.id for p in PANELS]: break
                            msg=await ws.recv()
...
async def do_sms_hit(bot_app,db_obj,otp_code,msg_body,service_name,panel_name,num_raw,session):
    ...
    # ©By tag — extract username from the number-bot link (e.g. @PAKOTPBOT)
    # Falls back to BOT_USERNAME if no number-bot link is configured
    _nb_link = NUMBER_BOT_LINK or GET_NUMBER_URL or ""
    if "t.me/" in _nb_link:
        _nb_user = _nb_link.rstrip("/").split("t.me/")[-1].lstrip("@")
        bot_tag  = f"@{_nb_user}" if _nb_user else f"@{BOT_USERNAME}"
    else:
        bot_tag  = f"@{BOT_USERNAME}" if BOT_USERNAME else "@PAKOTPBOT"

    # Number display: "92-PAKOTPBOT-7623" → shortened to fit
    bot_short   = bot_tag.lstrip("@")[:9].upper()   # e.g. "PAKOTPBOT"
    num_display = f"{dial}•{bot_short}•{last5}"

    # ══════════════════════════════════════════════════════
    #  DM MESSAGE  — premium format for private chat
    # ══════════════════════════════════════════════════════
    if clean:
        dm_txt = (
            f"<b>━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            f"  {flag} <b>#{svc}</b>  <code>{num_display}</code>  🔥\n"
            f"<b>━━━━━━━━━━━━━━━━━━━━━━</b>\n\n"
            f"<b>{header}</b>  ·  {count_badge}\n\n"
            f"🔑 <b>OTP:</b>  <code>{clean}</code>\n\n"
            f"💬 <i>{html.escape(msg_body[:280])}</i>\n\n"
        ...
        dm_txt = (
            ...
            f"<b>━━━━━━━━━━━━━━━━━━━━━━</b>\n\n"
            f"<b>{header}</b>\n\n"
            f"💬 <i>{html.escape(msg_body[:360])}</i>\n\n"
            f"<i>©By {html.escape(bot_tag)}</i>"
        )

    # ══════════════════════════════════════════════════════
    #  GROUP MESSAGE  — exact screenshot layout
    #  Line 1:  🇮🇪 #CI  📱  2250•PAKOTPBOT•7623  🔥
    #  Line 2:  (blank)
    #  Line 3:  ©By @PAKOTPBOT
    # ══════════════════════════════════════════════════════
    if clean:
        ...
        grp_txt = (
            f"{flag} <b>#{svc}</b>  📱  <code>{num_display}</code>\n\n"
            f"<b>{header}</b>\n"
            f"💬 <i>{html.escape(msg_body[:180])}</i>\n\n"
            f"<i>©By {html.escape(bot_tag)}</i>"
        )
    ...
    # ── Log groups — compact reference format ─────────────
...
async def log_unassigned(bot_app,num_raw,msg_body,otp_code,service_name,panel_name):
    ...
    svc=get_service_short(service_name)
    clean=re.sub(r"[^0-9]","",otp_code) if otp_code else ""
    _nb_link2 = NUMBER_BOT_LINK or GET_NUMBER_URL or ""
    if "t.me/" in _nb_link2:
        _nb_u2   = _nb_link2.rstrip("/").split("t.me/")[-1].lstrip("@")
        bot_tag2 = f"@{_nb_u2}" if _nb_u2 else f"@{BOT_USERNAME}"
    else:
        bot_tag2 = f"@{BOT_USERNAME}" if BOT_USERNAME else "@PAKOTPBOT"
    bot_short2  = bot_tag2.lstrip("@")[:9].upper()
    num_display2= f"{dial}•{bot_short2}•{last5}"

    if clean:
        txt = (
            f"{flag} <b>#{svc}</b>  📱  <code>{num_display2}</code>  🔥\n\n"
            f"🔑 <code>{clean}</code>\n\n"
            f"<i>©By {html.escape(bot_tag2)}</i>"
        )
    else:
        txt = (
            f"{flag} <b>#{svc}</b>  📱  <code>{num_display2}</code>\n\n"
    ...
    for gid in log_chats:
        try: await bot_app.bot.send_message(chat_id=gid,text=txt,reply_markup=kb,parse_mode="HTML")
...
async def active_watcher(application):
    ...
    for host, group in login_groups.items():
        ...
        for panel in group:
            ...
            if ok:
                await update_panel_login(panel.id, panel.sesskey, panel.api_url, True)
                            ...
                            if panel.panel_type == "login":
                                ...
                                if ok:
                                    await update_panel_login(
                                        panel.id, panel.sesskey, panel.api_url, True)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot_name = f"@{BOT_USERNAME}" if BOT_USERNAME else "@SigmaFetcherBot"
    ...
    msg = (
        "<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
        f"💎 <b>SIGMA FETCHER V10</b>  |  🤖 {html.escape(bot_name)}\n"
        "<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n\n"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="send_test_otp":
        ...
        await query.answer()
        bot_tag=f"@{BOT_USERNAME}" if BOT_USERNAME else "SigmaFetcher"
        now_ts=datetime.now().strftime("%H:%M:%S")
        test_txt=(
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
